...
## 简介
本文件聚焦于多智能体客服系统中的速率限制中间件，系统性阐述其在高并发场景下如何控制 LLM 调用频率与资源消耗。该中间件采用令牌桶算法实现平滑限流与突发能力，支持每秒补充令牌与容量上限控制，有效避免高并发时超出 API 速率限制。文档详细解释限流算法的实现原理、配置要点，并结合实际业务场景给出限流策略、阈值设定、最佳实践与动态调整策略。同时提供监控方法与故障排查建议，帮助读者在保证系统稳定性的同时提升用户体验。
...
## 核心组件
- **令牌桶 TokenBucket**：实现平滑限流与短时突发能力，支持每秒补充令牌与容量上限控制，默认速率10.0次/秒，容量20个令牌。
- **限流中间件 RateLimiterMiddleware**：在节点执行前进行令牌获取，超时则抛出运行时错误，阻断过载请求，默认超时时间为30秒。
...
- **作用范围**：仅对包含 LLM 调用的节点生效，包括意图分类、画像提取、技术支持、订单服务、产品咨询、质量检查等节点。
- **执行时机**：在节点执行前进行令牌获取，超时则立即中断，防止资源过度消耗。
- **错误处理**：限流本身不处理异常，交由异常处理中间件统一兜底。
- **配置参数**：速率（每秒允许的 LLM 调用次数，默认10.0）与容量（允许的突发令牌数，默认20），超时时间默认30秒。
...
- **业务代理**：技术支持、订单服务、产品咨询等执行具体业务逻辑，通常包含工具调用与 LLM 交互，需受限流保护。
...
- **速率（rate）**：每秒允许的 LLM 调用次数，默认值10.0，适用于一般并发场景。
- **容量（capacity）**：令牌桶容量，默认20，允许短时间突发，避免瞬时高峰被过度抑制。
- **超时时间**：默认30秒，防止长时间阻塞影响用户体验。
...
- **令牌桶优势**：平滑限流与突发能力兼顾，避免突发流量导致的系统抖动，默认配置可处理中等并发场景。
- **并发安全**：锁保护令牌桶状态，确保多线程环境下的正确性。
- **超时控制**：获取令牌设置超时时间，默认30秒，防止长时间阻塞影响用户体验。
...
- **限流超时**：当节点等待令牌超过超时时间（默认30秒）时抛出运行时错误，检查速率与容量配置是否合理，观察系统并发情况。
...
## 结论
速率限制中间件通过令牌桶算法在多智能体客服系统中实现了对 LLM 调用的精细化控制，既保障了系统在高并发场景下的稳定性，又维持了良好的用户体验。默认配置（10.0次/秒速率，20容量，30秒超时）经过系统测试验证，适用于一般并发场景。结合日志、计时与异常处理中间件，系统形成了完善的可观测性与容错机制。通过合理的阈值设定与动态调整策略，可在不同业务场景下实现最优的性能与稳定性平衡。

## 附录
- **算法选择**：令牌桶算法适用于需要平滑限流与突发能力的场景；漏桶算法更适合固定速率输出的场景。本系统采用令牌桶以更好地适配 LLM 调用的不确定性。
- **配置建议**：根据 API 限额与系统资源设定初始速率与容量，结合监控数据进行迭代优化。在高峰期可适当提高容量以应对突发，低峰期降低速率以节省成本。
- **动态调整**：在生产环境中可根据实际使用情况进行动态调整，建议通过监控指标（平均响应时间、错误率、CPU使用率）来指导配置优化。
- **扩展性**：系统设计支持自定义限流参数，可通过修改 RateLimiterMiddleware 的构造参数来适应不同的业务需求。