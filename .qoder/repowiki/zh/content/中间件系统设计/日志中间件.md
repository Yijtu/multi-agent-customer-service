...
**更新** 本版本反映了结构化日志中间件的完整实现，包括节点特定标签、自动跟踪记录和统一日志风格。
...
- 日志格式规范
  ...
  - **新增** 节点特定标签：使用 NODE_LABELS 映射表为每个节点提供人类友好的显示标签，包含 emoji 和中文描述。
- 日志级别与输出
  - INFO：节点开始与完成（含摘要）。
...
**更新** 日志中间件现在包含完整的节点标签系统，提供统一的日志风格和更好的可读性。
...
**更新** 新版本的日志中间件通过节点特定标签系统和统一的日志风格，显著提升了系统的可观测性和调试体验，为多智能体客服系统的稳定运行提供了强有力的支撑。
...
- 节点标签系统
  - **新增** NODE_LABELS 映射表：为每个节点提供人类友好的显示标签，包含 emoji 和中文描述。
  - 支持的节点标签：意图分类、画像提取、技术支持、订单服务、产品咨询、人工升级、质量检查、质量升级。
- 摘要提取规则
  - 意图分类：意图与置信度。
  - 画像提取：当前用户画像。
  - 质量检查：质量评分。
...
- 自定义日志格式
  - 可通过修改日志中间件的 NODE_LABELS 映射表，添加新的节点标签。
...
- **新增** 节点标签可视化
  - 通过 NODE_LABELS 映射表，提供直观的节点标识，提升日志可读性。
