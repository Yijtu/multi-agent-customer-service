...
- agents：业务Agent基类与具体Agent（技术、订单、产品）。
...
## 结论
BaseBusinessAgent基类通过模板方法模式与约定式配置，实现了业务Agent的高度一致性与可扩展性。配合用户画像增强、Hand-off机制与系统级中间件，系统在保证稳定性的同时，提供了良好的个性化与可维护性。建议在扩展新Agent时严格遵循“工具+提示词+回退”的三要素约定，并在Hand-off与错误处理上保持一致的策略与可观测性。