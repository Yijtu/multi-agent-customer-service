...
## 更新摘要
**变更内容**
- 新增完整的调用链追踪工具模块，提供节点执行链路记录功能
- 增强日志中间件，集成追踪工具进行统一的可观测性管理
- 新增UI格式化功能，支持Streamlit前端展示追踪结果
- 完善中间件链的可观测性闭环，实现从数据处理到可视化展示的完整监控体系
...
- **新增**：调用链追踪工具的完整实现，包括节点执行记录、格式化输出和UI展示
- 工具函数的模块化设计与依赖关系
- 使用示例与最佳实践
- 扩展开发指南与测试方法
- 在系统中的应用场景与性能考虑
...
- **新增**：调用链追踪工具：在 state["metadata"]["trace"] 中记录完整的节点执行链路，支持文本格式化和UI格式化两种输出方式
...
- **新增**：调用链追踪工具：在 state["metadata"]["trace"] 中记录节点执行链路，包含节点名、开始/结束时间、耗时、状态、摘要与错误信息，支持文本格式化和UI格式化两种输出
...
## 架构总览
系统采用 LangGraph 工作流编排，通过中间件链对节点执行进行统一观测与保护。工具函数库中的 JSON 解析与追踪工具分别服务于数据处理与可观测性两大方向。追踪工具提供完整的节点执行链路记录，支持文本格式化输出用于日志查看，以及UI格式化输出用于前端展示。
...
- **新增**：提供三种核心功能：
  - `create_trace_entry()`：创建单条 trace 记录，包含节点名、时间戳、耗时、状态、摘要和错误信息
  - `format_trace()`：将 trace 列表格式化为可读的文本字符串，适合命令行与日志查看
  - `format_trace_for_ui()`：将 trace 格式化为适合前端展示的结构，直接返回 trace 列表
- 数据结构：每条 trace entry 包含节点名、开始/结束时间、耗时、状态、摘要与错误信息
- 输出格式：
  - 文本格式：适合命令行与日志查看，包含状态图标、节点名、耗时、摘要和错误信息
  - UI格式：适合前端展示，直接返回 trace 列表，便于Streamlit等框架渲染
...
- **增强**：现在集成了调用链追踪工具，自动创建和管理 trace 记录
...
- **新增**：调用链追踪
  ...
  - 最佳实践：为关键节点添加摘要信息，便于快速定位问题；使用 format_trace_for_ui() 支持前端展示
...
- 扩展开发
  ...
  - **新增**：扩展追踪维度：在 trace 中增加自定义字段（如用户 ID、会话 ID），并在中间件中写入
  - **新增**：UI展示扩展：为 format_trace_for_ui() 添加新的字段映射，支持更丰富的前端展示
  - 新增工具函数：遵循 @tool 装饰器规范，返回结构化 JSON 字符串，并在 data/database.py 中实现对应查询
- 测试方法
  - 单元测试：针对 JSON 解析工具编写边界测试用例（Markdown 包裹、非法 JSON、空字符串等）
  - **新增**：追踪功能测试：验证 create_trace_entry() 的正确性、format_trace() 的格式化效果、format_trace_for_ui() 的数据结构
...
- **新增**：中间件链依赖 utils/tracer.py 提供的追踪工具进行节点执行记录
...
- **新增**：追踪与日志：仅在关键节点写入 trace，减少内存占用；格式化输出可在需要时按需调用
- **增强**：追踪性能：format_trace() 和 format_trace_for_ui() 都是纯函数，无副作用，性能开销极小
...
- **新增**：追踪功能异常
  - 现象：trace 记录缺失或格式化输出异常
  - 排查：检查中间件链配置，确认 LoggingMiddleware 正确调用 create_trace_entry()；验证 state["metadata"]["trace"] 结构
  - 参考路径：[middleware/logging_mw.py:65-76](file://middleware/logging_mw.py#L65-L76), [utils/tracer.py:32-68](file://utils/tracer.py#L32-L68)
- 节点异常
  - 现象：异常中间件记录错误并设置兜底回复与升级标志
  - 排查：查看日志中间件输出的 trace 与摘要信息，定位异常节点
  - 参考路径：[middleware/error_handler_mw.py:46-65](file://middleware/error_handler_mw.py#L46-65), [middleware/logging_mw.py:78-106](file://middleware/logging_mw.py#L78-106)
- 性能瓶颈
  - 现象：节点耗时较长
  - 排查：查看中间件计时输出与 trace 总耗时，识别热点节点
  - 参考路径：[middleware/timing_mw.py:20-43](file://middleware/timing_mw.py#L20-43), [utils/tracer.py:32-68](file://utils/tracer.py#L32-68)

**章节来源**
- [utils/json_parser.py:46-51](file://utils/json_parser.py#L46-L51)
- [middleware/error_handler_mw.py:46-65](file://middleware/error_handler_mw.py#L46-65)
- [middleware/logging_mw.py:78-106](file://middleware/logging_mw.py#L78-106)
- [middleware/timing_mw.py:20-43](file://middleware/timing_mw.py#L20-43)
- [utils/tracer.py:32-68](file://utils/tracer.py#L32-68)

## 结论
工具函数库通过 JSON 解析与调用链追踪两大能力，为系统提供了稳健的数据处理与可观测性支撑。**新增的调用链追踪工具**不仅提供了完整的节点执行记录功能，还支持文本格式化和UI格式化两种输出方式，为系统的调试、监控和前端展示提供了全面的解决方案。配合中间件链的统一日志、计时与异常处理，系统在复杂多轮对话场景中保持稳定与可维护性。建议在扩展新功能时遵循现有模块化设计，确保可观测性与性能不受影响。
...
- **新增**：追踪工具API参考路径：[utils/tracer.py:11-78](file://utils/tracer.py#L11-L78)
- **新增**：日志中间件追踪集成参考路径：[middleware/logging_mw.py:65-105](file://middleware/logging_mw.py#L65-L105)