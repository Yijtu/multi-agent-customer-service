...
## 简介
本指南面向从开发到生产的全生命周期部署运维，覆盖环境配置、依赖管理、数据库部署与初始化、Web UI 部署、负载均衡与高可用、监控与日志、性能调优、备份恢复与灾难恢复、版本发布与滚动更新等主题。系统基于 LangChain 1.0 + LangGraph，采用 SQLite 作为本地持久化存储，支持多轮对话状态（用户画像）跨轮次累积，并提供 Streamlit Web UI 与命令行演示入口。**新增**多语言支持配置，涵盖中英文日韩语种的部署考虑。
...
**图表来源**
- [main.py:1-153](file://main.py#L1-L153)
...
## 核心组件
- 配置中心：集中管理环境变量、模型初始化、阈值常量、SQLite 路径等，**新增**多语言支持配置（SUPPORTED_LANGUAGES、DEFAULT_LANGUAGE）。
...
## 架构总览
系统采用"工作流编排 + 代理协作 + 中间件增强"的分层架构。LangGraph 负责状态流转与路由，各 Agent 负责具体业务能力，中间件负责可观测性与稳定性保障。数据层通过 SQLite 提供业务数据持久化，Checkpointer 支持跨轮次状态恢复。**新增**RAG知识库与多语言支持集成。
...
RAG["RAG知识库 (Chroma + Embeddings)"]
LANG["多语言支持 (SUPPORTED_LANGUAGES)"]
CLIENT --> WEB
CLIENT --> CLI
WEB --> SYS
CLI --> SYS
...
- [config.py:43-75](file://config.py#L43-L75)
...
- **新增**多语言支持配置：SUPPORTED_LANGUAGES（支持的语言列表）和 DEFAULT_LANGUAGE（默认回复语言）。

部署要点
- 开发环境：复制 .env.example 为 .env，填入有效 API Key。
- 生产环境：通过环境变量注入，避免硬编码；确保路径权限可写。
- 容器化：通过镜像构建时注入环境变量或挂载卷。
- **新增**多语言部署：确保支持的语言包已安装，配置正确的语言检测逻辑。
...
- **新增**RAG知识库：Chroma 向量数据库持久化路径和嵌入模型配置。

```mermaid
flowchart TD
Start(["启动"]) --> InitDB["初始化业务数据库<br/>创建表"]
InitDB --> Seed["导入种子数据<br/>订单/产品/FAQ"]
Seed --> InitRAG["初始化RAG知识库<br/>Chroma向量数据库"]
InitRAG --> Ready["数据库就绪"]
...
- [config.py:53-67](file://config.py#L53-L67)
...
- **新增**RAG依赖：chromadb、langchain-chroma、langchain-huggingface、sentence-transformers、langchain-text-splitters。
...
REQ --> HF["langchain-huggingface"]
REQ --> STB["sentence-transformers"]
REQ --> LTS["langchain-text-splitters"]
CFG["config.py"] --> LC
SYS["system.py"] --> LG
SYS --> LCG
SYS --> SA
...
- **新增**RAG性能：向量检索优化、嵌入模型选择、缓存策略。
- **新增**多语言性能：语言检测开销、翻译服务成本、缓存机制。
...
- **新增**RAG故障：向量数据库连接失败、嵌入模型加载错误、知识库索引问题。
- **新增**多语言问题：语言检测失败、翻译服务异常、字符编码问题。
...
## 结论
本项目提供了从开发到生产的完整参考实现：清晰的分层架构、完善的中间件可观测性、可扩展的状态与数据持久化方案。生产部署建议结合真实数据库、反向代理与负载均衡，强化监控与限流策略，并制定备份与灾难恢复预案。**新增**多语言支持和RAG知识库为系统提供了更强大的国际化能力和智能问答能力。
...
- **新增**多语言配置：确保SUPPORTED_LANGUAGES中的语言包已安装。
...
- **新增**RAG数据：挂载向量数据库目录（CHROMA_DB_PATH）。
- **新增**多语言支持：安装必要的语言包和字体。
...
- **新增**RAG集群：向量数据库集群部署，支持水平扩展。
- **新增**多语言缓存：分布式缓存语言检测结果，提高响应速度。
...
- **新增**RAG优化：向量维度选择、相似度阈值调整、批量检索优化。
- **新增**多语言优化：语言检测缓存、翻译服务降级策略。
...
- **新增**RAG备份：向量数据库快照、嵌入模型权重备份。
- **新增**多语言数据：语言检测规则、翻译缓存数据备份。
...
- **新增**多语言版本：确保新版本支持所有配置的语言包。
- **新增**RAG版本：向量数据库兼容性检查，索引导入迁移脚本。

**章节来源**
- [config.py:70-75](file://config.py#L70-L75)

### 多语言部署考虑
- 语言支持：SUPPORTED_LANGUAGES 配置支持中英日韩四种语言。
- 默认语言：DEFAULT_LANGUAGE 设置为中文（zh）。
- 语言检测：在意图分类和画像提取阶段集成语言检测。
- 翻译服务：可选的翻译中间件，将非默认语言转换为默认语言处理。
- 字符编码：确保 UTF-8 编码支持，处理多字节字符。
- UI本地化：Streamlit 组件的多语言界面适配。

**章节来源**
- [config.py:68-75](file://config.py#L68-L75)
- [middleware/logging_mw.py:19-30](file://middleware/logging_mw.py#L19-L30)